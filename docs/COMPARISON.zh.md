...
1. **基线已修正**：这些结果是在修复 `zpayload-dumper` 的并发语义后重新测量的，
   `--concurrency` 现在会真正限制 worker 数，而不是隐式扩展到 CPU 线程数。

2. **两边都会扩展，但形状并不一致**：

   | Payload | zpayload-dumper 最佳加速 | payload-dumper-go 最佳加速 |
   |---------|--------------------------|----------------------------|
...
3. **当前交叉点**：在最终 5 轮矩阵里，Zig 只在 `bench32 c=1` 上领先。
   其余所有测点都是 Go 更快，只是在大 payload 的高并发区间差距会缩小。

4. **当前优化目标**：Zig 剩余瓶颈仍然在执行引擎本身。这轮重构改善了并发行为，
   但如果要在端到端结果上反超 Go，还需要继续压 direct-write 快路径、pending 所有权转移，
   以及 spill 触发率。
