...
3. **绝对性能差距仍在**：即使修复了并发语义，Go 实现仍然在所有重测点上更快。
   高并发时差距会缩小，尤其是大 payload，但还没有反转。

4. **当前优化目标**：现在剩余的瓶颈已经是 Zig 执行引擎本身，而不是 benchmark 语义。
   最可疑的部分是每个操作的缓冲区分配、pending 队列复制、线性 flush，以及 spill 文件 I/O。
