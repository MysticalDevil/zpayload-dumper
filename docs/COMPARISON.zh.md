...
- **工作线程**：`max(concurrency, cpu_count)` 个线程（默认 4，但不超过 CPU 核心数）
...
1. **单线程绝对优势**：Zig 在 c=1 时始终快 **4–5×**。
   这得益于零分配解压、无 GC 暂停以及直接的 `std.Io` 流式写入。
...
2. **并发扩展差距**：
   ...
   |---------|------------------------|---------------------------|
   | bench32 | 1.0×（无提升） | **4.5×** |
   | bench128 | 1.0×（无提升） | **4.6×** |
   | bench256 | 1.0×（无提升） | **4.0×** |
   | bench512 | 1.1×（几乎无提升） | **4.1×** |

   **zpayload-dumper 不随并发度扩展。** 从 1 个工作者到 8 个工作者，墙上时钟时间保持不变，而 Go 实现了 4× 的减少。

3. **交叉点**：小 payload（bench32）中，Go 在 c=4 时追平并在 c=8 时反超。大 payload（bench512）中，Zig 即使在 c=8 时仍领先约 14%。

4. **内存**：Zig 的内存占用约为 Go 的 **2×**。待处理队列的 256 MB 内存预算是主要因素。
...
- **运行时**：Zig 标准库 + 系统 C 库（`upb`、`utf8_range`、`lzma`、`bz2`、`zstd`）。
- **构建**：`zig build`（单条命令，但要求系统库已安装）。
...
# Zig：要求系统 C 库已安装
zig build   # 若缺少 upb、lzma、bz2、zstd 则失败
```

Go 版本通过纯 Go 或 cgo 包装模块（`gozstd`、`go-xz`、标准库 `compress/bzip2`）打包了所有压缩库。Zig 版本要求用户先安装原生系统库（`upb`、`utf8_range`、`lzma`、`bz2`、`zstd`）才能编译。
...
zig build -Dtarget=x86_64-windows-gnu   # 需要 Windows 版 upb/lzma/bz2/zstd
```

Go 的纯 Go 依赖使交叉编译 seamless。Zig 的 C 依赖（`upb`、`lzma` 等）需要这些库的交叉编译版本，或为目标平台从源码构建。
