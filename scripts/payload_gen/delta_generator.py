...
from payload_gen.manifest import load_update_metadata_pb2
from payload_gen.models import Extent, Operation, PartitionSpec
from payload_gen.paths import PROTO_DIR, REPO_ROOT
from payload_gen.utils import pad_block_aligned
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.bundle_dir is not None:
        spec = PartitionSpec(
            ...
            Operation(
                "SOURCE_BSDIFF",
                [Extent(0, (len(new_data) + args.block_size - 1) // args.block_size)],
                patch,
            ),
        )
        write_bundle(
            ...
            scenario_lines=[
                ...
                f"partition_name={args.partition_name}",
                "expected=success",
            ],
            expected_text="success\n",
            old_images={args.partition_name: old_data},
        )
        print(f"[OK] fixture bundle written to {args.bundle_dir}")
        print(f"[INFO] old images: {args.bundle_dir / 'old'}")
        print(f"[INFO] expected extracted images: {args.bundle_dir / 'extracted'}")
