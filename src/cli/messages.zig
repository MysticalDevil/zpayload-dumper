...
pub fn userMessage(err: Error) []const u8 {
    return switch (err) {
        ...
        error.ZipDecompressTruncated => "zip archive entry is truncated or corrupt",
        error.PayloadNotFoundInZip => "payload.bin not found in zip archive",
        error.InvalidTarArchive => "invalid tar archive or failed to read tar content",
        error.PayloadNotFoundInTar => "payload.bin not found in tar archive",
        error.TarDecompressTruncated => "tar archive truncated or corrupt",
        error.TempDirectoryUnavailable => "no usable temporary directory is available for archive extraction",
        error.TempDirectoryCreateFailed => "failed to create the temporary extraction directory",
