...
fn suggestionForError(err: Error) ?[]const u8 {
    return switch (err) {
        ...
        error.ZipDecompressTruncated => "re-download the zip archive and retry extraction",
        error.PayloadNotFoundInZip => "ensure the zip archive contains payload.bin",
        error.InvalidTarArchive => "verify the file is a valid tar or tar.gz archive",
        error.PayloadNotFoundInTar => "ensure the tar archive contains payload.bin",
        ...
        error.ArchiveWriteFailed => "verify the temporary directory is writable and has enough free space",
